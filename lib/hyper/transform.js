...
(function() {
  var CoffeeScript, IDENTIFIER, IS_STRING, interpolate, interpolatingNode, isStringNode, isTagNode, isWrappedNode, nodewalk, normalizeComponentCalls, potentialTags, tags, tagsToImport,
  ...
  normalizeComponentCalls = function(source) {
    ...
    normalize = function(source) {
      return source.replace(/((?:^|\s|\#\{)(\s*)_\w+)(\s*\n\2[^\S\n]+)([^\n]+)/g, function(match, tag, _, indent, following) {
        var identifier;
        if ((identifier = following.match(IDENTIFIER)) && identifier[2]) {
          return match;
        } else {
